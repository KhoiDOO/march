...
namespace grid
{
    ...
    struct PointToBBox
    {
        __device__ __host__
            BBox<Scalar>
            operator()(const Vertex<Scalar> &pt) const
        {
    ...
    struct BBoxReduce
    {
        __device__ __host__
            BBox<Scalar>
            operator()(const BBox<Scalar> &a, const BBox<Scalar> &b) const
        {
            return a.merge(b);
        }
    };
        ...
        IndexType core_end_x, IndexType core_end_y, IndexType core_end_z)
    {
        ...
        if (voxel_idx >= p_res_x * p_res_y * p_res_z)
            return;

...
namespace mc
{
    ...
    struct TriCountOp
    {
        __device__ int operator()(const uint8_t code) const
        {
    ...
    void MC<Scalar, IndexType>::ensure_used_voxel_storage_size(size_t n_used_voxels)
    {
        if (n_used_voxels > this->allocated_used_voxel_count)
        {
            ...
            if (this->used_to_first_mc_tri)
                CHECK_CUDA(cudaFree(this->used_to_first_mc_tri));
        ...
        uint8_t *voxel_codes)
    {
        ...
        for (int i = 0; i < 8; ++i)
        {
            if (values[v_ptr[mc_local_v_idx[i]]] >= iso)
            {
                code |= (1 << i);
            }
        }
        ...
        IndexType n_used_voxels)
    {
        ...
        if (active_voxel_idx >= n_used_voxels)
            return;
        ...
        for (int i = 0; i < face_num; ++i)
        {
            ...
            IndexType v0 = v_ptr[mc_edge2vertices[local_edge_id][0]];
            IndexType v1 = v_ptr[mc_edge2vertices[local_edge_id][1]];
        ...
        IndexType n_verts)
    {
        ...
        for (int i = 0; i < 12; ++i)
        {
            IndexType v0 = v_ptr[mc_edge2vertices[i][0]];
            IndexType v1 = v_ptr[mc_edge2vertices[i][1]];
        ...
        bool with_colors)
    {
        ...
        if (with_colors)
        {
            const Scalar EPS = Scalar(1e-5);
            Scalar t = Scalar(0.5);
            if (d_abs(iso - v0_val) < EPS)
                t = Scalar(0.0);
            else if (d_abs(iso - v1_val) < EPS)
                t = Scalar(1.0);
